<?php
    ...
    function sec_session_start() { // Uma função que vai iniciar uma sessão com medidas de segurança adicionais.
    ...
    function login($email, $password, $mysqli) {
        // Usando definições pré-estabelecidas significa que a injeção de SQL (um tipo de ataque) não é possivel.
        if($stmt = $mysqli->prepare("SELECT id, username, password, salt
            ...
            LIMIT 1")) {
                ...
                if($stmt->num_rows == 1) {
                    // Caso o usuário exita, conferimos se a conta está bloqueada
    ...
    function login_check($mysqli) {
        ...
        if(isset($_SESSION['user_id'],
           ...
           if($stmt = $mysqli->prepare("SELECT password
               ...
               if($stmt->num_rows == 1) {
                 // Caso o usuário exista, pega as variáveis a partir do resultado.
    ...
    function esc_url($url) {
        ...
        if($url[0] !=='/') {
            // Estamos interessados somente em links relacionados provenientes de $SERVER['PHP_SELF']
            return '';
        }else {
            return $url;
        }
